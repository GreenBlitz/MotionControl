...
public class MotorManager {	
	...
	public static ChainAssembly build(IChainSupplier<Double> desiredVInput,
									  IChainSupplier<Double> actualVInput,
									  VoltageController voltageController,
									  IChainConsumer<Double, Boolean> actuator,
									  ChainAssembly factory,
									  String suffix){
		...
		factory.connection("desiredToVoltage " + suffix)
		.supplierFor("desired " + suffix)
		.consumerFor("voltage " + suffix);
		
		factory.connection("actualToVoltage " + suffix)
		.supplierFor("velocity " + suffix)
		.consumerFor("voltage " + suffix);
		
		factory.connection("voltageToPower " + suffix)
		.conversion((voltage) -> (Double)voltage.get(0) / 12.0)
