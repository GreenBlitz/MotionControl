package gbmotion.base.point.orientation;
...
public interface IOrientation2D extends IPoint2D {
	...
	default IOrientation2D changePrespectiveTo(IOrientation2D origin) {
		return (IOrientation2D) moveByReversed(origin, DirectionEffect.IGNORED).rotate(-origin.getDirection(),
	...
	default IOrientation2D rotateAround(IOrientation2D origin, double angle, DirectionEffect effect) {
		...
		switch (effect) {
		case CHANGED:
			IOrientation2D relative = Orientation2D
					.mutable(getX() - origin.getX(), getY() - origin.getY(), getDirection()).rotate(angle);
			return set(relative.getX() + origin.getX(), relative.getY() + origin.getY(), relative.getDirection());
		case RESERVED:
		case IGNORED:
			return ((IOrientation2D) moveByReversed(origin, DirectionEffect.IGNORED).rotate(angle, effect))
	...
	default IPoint2D rotateAround(IPoint2D origin, double angle) {
		...
		return ((IOrientation2D) moveByReversed(origin.getX(), origin.getY(), 0, effect).rotate(angle, effect))
				.moveBy(origin.getX(), origin.getY(), 0, effect);
	}
	...
	default void toDashboard(String name, EnvironmentPort ePort) {
		ePort.putNumber(name + " X coordinate", getDirection());
		ePort.putNumber(name + " Y coordinate", getDirection());
		ePort.putNumber(name + " direction", getDirection());
	}
}
